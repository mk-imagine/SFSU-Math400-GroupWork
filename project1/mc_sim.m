...
for i=1:numsims
    ...
    wait = 0;    % number of people who have to wait
    change = 0;  % number of people needing change
    fives = 0;   % number of fives in register
    for j=1:numppl
        prob = rand;
        % if person does not need change (has a five)
        if prob <= 0.5
            if change == 0          % if no one needs change
                fives = fives + 1;  % add to fives bucket
            else                    % else if someone needs change
                change = change - 1;% reduce number of people that need change (no fives go in bucket)
            end
        % else if person needs change (has a ten)
        else
            if fives == 0           % if fives bucket empty
                wait = wait + 1;    % person has to wait
                change = change + 1;% and add to number of people waiting for change
